...
model <- get_arg("--model", default = Sys.getenv("REVIEW_MODEL", "gemini-3.1-pro-preview"))
...
prompt_parts <- c(
  ...
  "",
  "When interpreting BiocCheck and R CMD check results, follow these severity mappings:",
  "- **BiocCheck/R CMD check ERRORs** → REQUIRED fixes that MUST be addressed before acceptance",
  "- **BiocCheck/R CMD check WARNINGs** → STRONGLY RECOMMENDED fixes that should be addressed",
  "- **BiocCheck/R CMD check NOTEs** → INFORMATIONAL suggestions only - do NOT classify as required",
  "",
  "IMPORTANT: BiocCheck NOTEs (e.g., suggestions to add NEWS file, consider shorter lines, suggestions about documentation for unexported functions) are informational feedback and should NOT be presented as required changes. These can be mentioned in a 'Suggestions' or 'Optional Improvements' section if relevant, but should never appear under 'Required' items.",
  "",
  "## Review Structure",
  "",
